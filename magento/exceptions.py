...
def build_exception_text(message: str, parameters: dict[str, str] | list[str] | None) -> str:
    ...
    if isinstance(parameters, dict):
        return RE_PARAM.sub(lambda m: str(parameters[m.group(1)]), message)

        # For example:
    #   message = 'El atributo "%1" no incluye una opción con el ID "%2".'
    #   parameters = ['manufacturer', '17726']
    if isinstance(parameters, list):
        return RE_PARAM.sub(lambda m: str(parameters[int(m.group(1)) - 1]), message)
